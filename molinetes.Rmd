...
df_analisis %>%
  ...
  theme_linedraw() +
  theme(legend.position = "none") + 
  scale_x_datetime(breaks= seq(min(df_linea$ts), max(df_linea$ts), by = "12 hour"), date_labels = "%a %H %M") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 5)) + 
  geom_vline(aes(xintercept=min(df_test$ts), color='red')) +
  geom_text(aes(x=min(df_test$ts) , label="TRAIN       TEST", y=1600), colour="red", angle=0)
