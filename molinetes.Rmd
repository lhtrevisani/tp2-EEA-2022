...
plot(reg)
```
predict en train y predict en test:
...
Calculo la exponencial de las predicciones:
...
```{r}
modelo = auto.arima(df_train %>% as_tsibble(index = ds) %>% fill_gaps() %>% select(residuos))
...
Analizo los residuos del arima (ploteo + histograma?):
